cd /ibex/scratch/projects/c2042/user_masrir
mkdir -p khudri_stitch/{lists,logs,stitch,dosage_chunks,results,plots,data/reference_genome}
cd khudri_stitch

nano build_khudri_bam_list.sh
#!/usr/bin/env bash
set -euo pipefail

WD=/ibex/scratch/projects/c2042/user_masrir/khudri_stitch
BAMDIR=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/results/mapped_dup_rm
OUT=${WD}/lists/list_of_bams_for_stitch_khudri_no_header.tsv

mkdir -p "${WD}/lists"

find "${BAMDIR}" -maxdepth 1 -type f \( -name '*khudri*high*.RG.bam' -o -name '*khudri*low*.RG.bam' \) \
  | sort -u > "${OUT}"

echo "Khudri BAM list written to: ${OUT}"
wc -l "${OUT}"
head "${OUT}"

bash build_khudri_bam_list.sh

nano prepare_stitch_inputs_khudri.sh
#!/usr/bin/env bash
set -euo pipefail
...
bash prepare_stitch_inputs_khudri.sh

nano stitch_khudri.slurm
#!/usr/bin/env bash
#SBATCH --job-name=stitch_khudri
#SBATCH --cpus-per-task=24
#SBATCH --mem=180G
#SBATCH --time=12:00:00
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/logs/stitch_%A_%a.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/logs/stitch_%A_%a.err
#SBATCH --export=ALL
...
BAMLIST=${WD}/lists/list_of_bams_for_stitch_khudri_no_header.tsv
...
nano submit_stitch_all_khudri.sh
#!/usr/bin/env bash
set -euo pipefail
...
for CHR in "${CHRS[@]}"; do
  ...
  sbatch --array=1-"${N}" --export=ALL,CHR="${CHR}" stitch_khudri.slurm
done

bash submit_stitch_all_khudri.sh

module load bcftools
bcftools query -l /ibex/scratch/projects/c2042/user_masrir/khudri_stitch/stitch/Chr01/chunk001_1_5000000/stitch.Chr01.1.5000000.vcf.gz > sample_order.txt
...
nano extract_dosage_khudri_all.sh
#!/usr/bin/env bash
set -euo pipefail
...
cat \
  ...
  > "${WD}/Khudri_whole_genome.dedup.dosage.tsv"
  
bash extract_dosage_khudri_all.sh

nano fisher_whole_genome_khudri.R
dos <- read.table("Khudri_whole_genome.dedup.dosage.tsv",
                  header = FALSE, sep = "\t",
                  stringsAsFactors = FALSE, check.names = FALSE)

grp <- read.table("sample_groups.tsv",
                  header = TRUE, sep = "\t",
...
write.table(out,
            file = "Khudri_whole_genome.fisher_results.tsv",
            sep = "\t",
            quote = FALSE,
            row.names = FALSE)
            
nano fisher_whole_genome_khudri.slurm
#!/usr/bin/env bash
#SBATCH --job-name=fisher_whole_genome_khudri
#SBATCH --cpus-per-task=1
#SBATCH --mem=240G
#SBATCH --time=24:00:00
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/logs/fisher_whole_genome_khudri_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/logs/fisher_whole_genome_khudri_%j.err
...
Rscript --vanilla fisher_whole_genome_khudri.R

sbatch fisher_whole_genome_khudri.slurm

nano Manhattan_Khudri_imputed.R
...
infile <- "/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/Khudri_whole_genome.fisher_results.tsv"
outfile <- "/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/plots/Khudri_whole_genome_manhattan.png"

dir.create("/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/plots",
...
p <- ggplot(df, aes(x = bp_cum, y = neglogp, color = factor(color_id))) +
  ...
  labs(
    ...
    title = "Khudri whole-genome Manhattan plot"
  ) +
  theme_classic() +
  theme(
...
nano Manhattan_Khudri_imputed.sh
#!/usr/bin/env bash
#SBATCH --job-name=manhattan_khudri
#SBATCH --cpus-per-task=1
#SBATCH --mem=32G
#SBATCH --time=06:00:00
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/logs/manhattan_khudri_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/logs/manhattan_khudri_%j.err
...
cd /ibex/scratch/projects/c2042/user_masrir/khudri_stitch
mkdir -p logs plots
...
Rscript --vanilla Manhattan_Khudri_imputed.R

sbatch Manhattan_Khudri_imputed.sh
