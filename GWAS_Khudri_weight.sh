...
mkdir -p logs
mkdir -p Rlib
mkdir -p gwas_khudri_weight_genomewide

cd /ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia
module load R/4.3.2/gnu-12.2.0

R_LIBS_USER=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/Rlib \
Rscript -e 'install.packages("qqman", repos="https://cloud.r-project.org")'

nano make_pheno_weight_khudri.sh
#!/bin/bash
set -euo pipefail

BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
SRC="${BASE}/pheno_weight_khudri.plink2.txt"
OUT="${BASE}/pheno_weight_khudri.iid.txt"
...
nano merge_and_plot_weight_genomewide.R
...
gwas$CHR <- suppressWarnings(as.integer(gsub("^chr", "", gwas$`#CHROM`, ignore.case = TRUE)))
gwas$BP  <- suppressWarnings(as.integer(gwas$POS))
gwas$SNP <- ifelse(is.na(gwas$ID) | gwas$ID == ".", paste0(gwas$CHR, ":", gwas$BP), gwas$ID)
...
chr_counts <- table(gwas$CHR)
cat("Chromosome counts:\n")
print(chr_counts)
...
write.table(
  gwas[, c("CHR", "BP", "SNP", "P")],
  file = file.path(outdir, "khudri_weight_manhattan_input.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
png(
  filename = file.path(outdir, "khudri_weight_genomewide_manhattan.png"),
  width = 2600,
  height = 1400,
  res = 220
)
...
manhattan(
  ...
  ylim = c(0, max_y + 0.5),
  chrlabs = sort(unique(as.character(gwas$CHR))),
  main = paste0(
    "Khudri seed weight GWAS (genome-wide)\nBonferroni threshold = ",
    signif(bonf_p, 3)
  )
)
...
writeLines(
  c(
    ...
    paste("Chromosomes in plot:", paste(sort(unique(gwas$CHR)), collapse = ","))
  ),
  con = file.path(outdir, "khudri_weight_plot_summary.txt")
)

cat("Done.\n")
cat("Bonferroni p-value:", bonf_p, "\n")
cat("Bonferroni line:", bonf_line, "\n")
cat("Plot written to:", file.path(outdir, "khudri_weight_genomewide_manhattan.png"), "\n")

nano run_all_gwas_weight.slurm
#!/bin/bash
#SBATCH --job-name=khudri_weight_genomewide
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/khudri_weight_genomewide_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/khudri_weight_genomewide_%j.err
...
BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
STITCH_BASE="/ibex/scratch/projects/c2042/user_masrir/khudri_stitch/stitch"
PHENO="${BASE}/pheno_weight_khudri.iid.txt"
OUTBASE="${BASE}/gwas_khudri_weight_genomewide"
MERGED="${OUTBASE}/khudri_weight_all_chunks.merged.glm.linear"
...
BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
MERGED="${BASE}/gwas_khudri_weight_genomewide/khudri_weight_all_chunks.merged.glm.linear"
OUTBASE="${BASE}/gwas_khudri_weight_genomewide"
PLOT_R="${BASE}/merge_and_plot_weight_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
