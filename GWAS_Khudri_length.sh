...
nano make_pheno_length_khudri.R
...
out_plink2 <- file.path(base, "pheno_length_khudri.plink2.txt")
...
kh <- df[tolower(df$cultivar) == "khudri" & !is.na(df$length), c("id", "length")]
names(kh) <- c("IID", "length")

kh <- merge(grp[, "sample_id", drop = FALSE], kh, by.x = "sample_id", by.y = "IID", all.x = TRUE)
kh <- kh[!is.na(kh$length), ]

out <- kh[, c("sample_id", "length")]
...
Rscript make_pheno_length_khudri.R

nano make_pheno_length_khudri.sh
#!/bin/bash
set -euo pipefail

BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
SRC="${BASE}/pheno_length_khudri.plink2.txt"
OUT="${BASE}/pheno_length_khudri.iid.txt"
...
bash make_pheno_length_khudri.sh

nano run_all_gwas_length.slurm
#!/bin/bash
#SBATCH --job-name=khudri_length_genomewide
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/khudri_length_genomewide_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/khudri_length_genomewide_%j.err
...
PHENO="${BASE}/pheno_length_khudri.iid.txt"
OUTBASE="${BASE}/gwas_khudri_length_genomewide"
MERGED="${OUTBASE}/khudri_length_all_chunks.merged.glm.linear"
PLOT_R="${BASE}/merge_and_plot_length_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
...
nano merge_and_plot_length_genomewide.R
...
write.table(
  ...
  file = file.path(outdir, "khudri_length_manhattan_input.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
png(
  filename = file.path(outdir, "khudri_length_genomewide_manhattan.png"),
  width = 3200,
  height = 1600,
  res = 240
)
...
manhattan(
  ...
  chrlabs = chr_levels,
  main = paste0(
    "Khudri seed length GWAS (genome-wide)\nBonferroni threshold = ",
    signif(bonf_p, 3)
  )
)
...
writeLines(
  ...
  con = file.path(outdir, "khudri_length_plot_summary.txt")
)

cat("Done.\n")
cat("Bonferroni p-value:", bonf_p, "\n")
cat("Bonferroni line:", bonf_line, "\n")
cat("Plot written to:", file.path(outdir, "khudri_length_genomewide_manhattan.png"), "\n")

nano plot_only_length.slurm
#!/bin/bash
#SBATCH --job-name=plot_length_only
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/plot_length_only_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/plot_length_only_%j.err
...
BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
MERGED="${BASE}/gwas_khudri_length_genomewide/khudri_length_all_chunks.merged.glm.linear"
OUTBASE="${BASE}/gwas_khudri_length_genomewide"
PLOT_R="${BASE}/merge_and_plot_length_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
...
sbatch run_all_gwas_length.slurm
sbatch plot_only_length.slurm
