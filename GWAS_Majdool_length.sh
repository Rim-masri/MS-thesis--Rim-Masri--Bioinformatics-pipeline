nano make_pheno_length_majdool.R
...
out_plink2 <- file.path(base, "pheno_length_majdool.plink2.txt")
...
lengthv <- suppressWarnings(as.numeric(df$length))

grp$sample_id <- trimws(as.character(grp$sample_id))

df$id <- paste(cultivar, quality, serial, sep = "_")

mj <- df[tolower(cultivar) == "majdool" & !is.na(lengthv), c("id", "length")]
names(mj) <- c("IID", "length")

mj <- merge(grp[, "sample_id", drop = FALSE], mj,
            by.x = "sample_id", by.y = "IID", all.x = TRUE)

mj <- mj[!is.na(mj$length), ]

out <- mj[, c("sample_id", "length")]
...
nano make_pheno_length_majdool.sh
#!/bin/bash
set -euo pipefail

BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
SRC="${BASE}/pheno_length_majdool.plink2.txt"
OUT="${BASE}/pheno_length_majdool.iid.txt"
...
Rscript make_pheno_length_majdool.R
bash make_pheno_length_majdool.sh

nano merge_and_plot_length_majdool_genomewide.R
...
write.table(
  ...
  file = file.path(outdir, "majdool_length_manhattan_input.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
png(
  filename = file.path(outdir, "majdool_length_genomewide_manhattan.png"),
  width = 3200,
  height = 1600,
  res = 240
)
...
manhattan(
  ...
  chrlabs = chr_levels,
  main = paste0(
    "Majdool seed length GWAS (genome-wide)\nBonferroni threshold = ",
    signif(bonf_p, 3)
  )
)
...
writeLines(
  ...
  con = file.path(outdir, "majdool_length_plot_summary.txt")
)

nano run_all_gwas_length_majdool.slurm
#!/bin/bash
#SBATCH --job-name=majdool_length_genomewide
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/majdool_length_genomewide_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/majdool_length_genomewide_%j.err
...
PHENO="${BASE}/pheno_length_majdool.iid.txt"
OUTBASE="${BASE}/gwas_majdool_length_genomewide"
MERGED="${OUTBASE}/majdool_length_all_chunks.merged.glm.linear"
PLOT_R="${BASE}/merge_and_plot_length_majdool_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
...
nano plot_only_length_majdool.slurm
#!/bin/bash
#SBATCH --job-name=plot_length_majdool_only
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/plot_length_majdool_only_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/plot_length_majdool_only_%j.err
...
BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
MERGED="${BASE}/gwas_majdool_length_genomewide/majdool_length_all_chunks.merged.glm.linear"
OUTBASE="${BASE}/gwas_majdool_length_genomewide"
PLOT_R="${BASE}/merge_and_plot_length_majdool_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
...
sbatch run_all_gwas_length_majdool.slurm
