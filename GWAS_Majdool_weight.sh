cd /ibex/scratch/projects/c2042/user_masrir/majdool_stitch/stitch

bcftools query -l ./Chr01/chunk001_1_5000000/stitch.Chr01.1.5000000.vcf.gz > sample_order_majdool.txt

awk 'BEGIN{print "sample_id"} {print $1}' sample_order_majdool.txt > sample_groups_majdool.tsv

nano make_pheno_weight_majdool.R
...
out_plink2 <- file.path(base, "pheno_weight_majdool.plink2.txt")
...
mj <- df[tolower(cultivar) == "majdool" & !is.na(df$weight_num), c("id", "weight_num")]
names(mj) <- c("IID", "weight")

mj <- merge(grp[, "sample_id", drop = FALSE], mj,
            by.x = "sample_id", by.y = "IID", all.x = TRUE)

mj <- mj[!is.na(mj$weight), ]

out <- mj[, c("sample_id", "weight")]
...
nano make_pheno_weight_majdool.sh
#!/bin/bash
set -euo pipefail

BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
SRC="${BASE}/pheno_weight_majdool.plink2.txt"
OUT="${BASE}/pheno_weight_majdool.iid.txt"
...
Rscript make_pheno_weight_majdool.R
bash make_pheno_weight_majdool.sh

nano merge_and_plot_weight_majdool_genomewide.R
...
write.table(
  ...
  file = file.path(outdir, "majdool_weight_manhattan_input.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
png(
  filename = file.path(outdir, "majdool_weight_genomewide_manhattan.png"),
  width = 3200,
  height = 1600,
  res = 240
)
...
manhattan(
  ...
  chrlabs = chr_levels,
  main = paste0(
    "Majdool seed weight GWAS (genome-wide)\nBonferroni threshold = ",
    signif(bonf_p, 3)
  )
)
...
writeLines(
  ...
  con = file.path(outdir, "majdool_weight_plot_summary.txt")
)

nano run_all_gwas_weight_majdool.slurm
#!/bin/bash
#SBATCH --job-name=majdool_weight_genomewide
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/majdool_weight_genomewide_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/majdool_weight_genomewide_%j.err
...
PHENO="${BASE}/pheno_weight_majdool.iid.txt"
OUTBASE="${BASE}/gwas_majdool_weight_genomewide"
MERGED="${OUTBASE}/majdool_weight_all_chunks.merged.glm.linear"
PLOT_R="${BASE}/merge_and_plot_weight_majdool_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
...
nano plot_only_weight_majdool.slurm
#!/bin/bash
#SBATCH --job-name=plot_weight_majdool_only
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/plot_weight_majdool_only_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/plot_weight_majdool_only_%j.err
...
BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
MERGED="${BASE}/gwas_majdool_weight_genomewide/majdool_weight_all_chunks.merged.glm.linear"
OUTBASE="${BASE}/gwas_majdool_weight_genomewide"
PLOT_R="${BASE}/merge_and_plot_weight_majdool_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
...
sbatch run_all_gwas_weight_majdool.slurm
