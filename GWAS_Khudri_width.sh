...
nano make_pheno_width_khudri.R
base <- "/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
book <- file.path(base, "Book1.csv")
groups <- file.path(base, "sample_groups.tsv")
out_plink2 <- file.path(base, "pheno_width_khudri.plink2.txt")
...
df$serial   <- trimws(as.character(df$serial))
df$cultivar <- trimws(as.character(df$cultivar))
df$quality  <- trimws(as.character(df$quality))
df$width    <- suppressWarnings(as.numeric(df$width))
grp$sample_id <- trimws(as.character(grp$sample_id))

df$id <- paste(df$cultivar, df$quality, df$serial, sep = "_")

kh <- df[tolower(df$cultivar) == "khudri" & !is.na(df$width), c("id", "width")]
names(kh) <- c("IID", "width")

kh <- merge(grp[, "sample_id", drop = FALSE], kh, by.x = "sample_id", by.y = "IID", all.x = TRUE)
kh <- kh[!is.na(kh$width), ]

out <- kh[, c("sample_id", "width")]
...
Rscript make_pheno_width_khudri.R

nano make_pheno_width_khudri.sh
#!/bin/bash
set -euo pipefail

BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
SRC="${BASE}/pheno_width_khudri.plink2.txt"
OUT="${BASE}/pheno_width_khudri.iid.txt"
...
bash make_pheno_width_khudri.sh

nano run_all_gwas_width.slurm
#!/bin/bash
#SBATCH --job-name=khudri_width_genomewide
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/khudri_width_genomewide_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/khudri_width_genomewide_%j.err
...
PHENO="${BASE}/pheno_width_khudri.iid.txt"
OUTBASE="${BASE}/gwas_khudri_width_genomewide"
MERGED="${OUTBASE}/khudri_width_all_chunks.merged.glm.linear"
PLOT_R="${BASE}/merge_and_plot_width_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
...
echo "Number of .width.glm.linear files found: ${NGLM}"

if [[ "${NGLM}" -eq 0 ]]; then
    echo "ERROR: no .width.glm.linear files were produced."
    exit 1
fi
...
nano merge_and_plot_width_genomewide.R
...
write.table(
  ...
  file = file.path(outdir, "khudri_width_manhattan_input.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
png(
  filename = file.path(outdir, "khudri_width_genomewide_manhattan.png"),
  width = 3200,
  height = 1600,
  res = 240
)
...
manhattan(
  ...
  chrlabs = chr_levels,
  main = paste0(
    "Khudri seed width GWAS (genome-wide)\nBonferroni threshold = ",
    signif(bonf_p, 3)
  )
)
...
writeLines(
  ...
  con = file.path(outdir, "khudri_width_plot_summary.txt")
)

cat("Done.\n")
cat("Bonferroni p-value:", bonf_p, "\n")
cat("Bonferroni line:", bonf_line, "\n")
cat("Plot written to:", file.path(outdir, "khudri_width_genomewide_manhattan.png"), "\n")

nano plot_only_width.slurm
#!/bin/bash
#SBATCH --job-name=plot_width_only
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/plot_width_only_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/plot_width_only_%j.err
...
BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
MERGED="${BASE}/gwas_khudri_width_genomewide/khudri_width_all_chunks.merged.glm.linear"
OUTBASE="${BASE}/gwas_khudri_width_genomewide"
PLOT_R="${BASE}/merge_and_plot_width_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
...
sbatch run_all_gwas_width.slurm
sbatch plot_only_width.slurm
