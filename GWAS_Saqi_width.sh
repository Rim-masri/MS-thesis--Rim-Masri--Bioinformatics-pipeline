...
out_plink2 <- file.path(base, "pheno_width_saqi.plink2.txt")
...
sq <- df[tolower(df$cultivar) == "saqi" & !is.na(df$width), c("id", "width")]
names(sq) <- c("IID", "width")

sq <- merge(grp[, "sample_id", drop = FALSE], sq, by.x = "sample_id", by.y = "IID", all.x = TRUE)
sq <- sq[!is.na(sq$width), ]

out <- sq[, c("sample_id", "width")]
...
Rscript make_pheno_width_saqi.R

nano make_pheno_width_saqi.sh
#!/bin/bash
set -euo pipefail

BASE="/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia"
SRC="${BASE}/pheno_width_saqi.plink2.txt"
OUT="${BASE}/pheno_width_saqi.iid.txt"
...
bash make_pheno_width_saqi.sh

nano merge_and_plot_width_saqi_genomewide.R
...
write.table(
  ...
  file = file.path(outdir, "saqi_width_manhattan_input.tsv"),
  sep = "\t",
  row.names = FALSE,
  quote = FALSE
)
...
png(
  filename = file.path(outdir, "saqi_width_genomewide_manhattan.png"),
  width = 3200,
  height = 1600,
  res = 240
)
...
manhattan(
  ...
  chrlabs = chr_levels,
  main = paste0(
    "Saqi seed width GWAS (genome-wide)\nBonferroni threshold = ",
    signif(bonf_p, 3)
  )
)
...
writeLines(
  ...
  con = file.path(outdir, "saqi_width_plot_summary.txt")
)

cat("Done.\n")
cat("Bonferroni p-value:", bonf_p, "\n")
cat("Bonferroni line:", bonf_line, "\n")
cat("Plot written to:", file.path(outdir, "saqi_width_genomewide_manhattan.png"), "\n")

nano run_all_gwas_width_saqi.slurm
#!/bin/bash
#SBATCH --job-name=saqi_width_genomewide
#SBATCH --output=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/saqi_width_genomewide_%j.out
#SBATCH --error=/ibex/scratch/projects/c2042/user_masrir/datepalm_metaxenia/logs/saqi_width_genomewide_%j.err
...
PHENO="${BASE}/pheno_width_saqi.iid.txt"
OUTBASE="${BASE}/gwas_saqi_width_genomewide"
MERGED="${OUTBASE}/saqi_width_all_chunks.merged.glm.linear"
PLOT_R="${BASE}/merge_and_plot_width_saqi_genomewide.R"
R_LIB_DIR="${BASE}/Rlib"
...
sbatch run_all_gwas_width_saqi.slurm
